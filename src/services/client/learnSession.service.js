...
const MODE_DEFAULTS = {
    ...
    [LEARN_SESSION_MODE.MASTER]: {
        ...
        masteryTarget: null // null -> bằng số bậc ladder bắt buộc
    }
}
...
// ---- Chọn phạm vi thẻ ----
// LƯU Ý: cardProgress hiện CHƯA có trường `starred` (NC12 - PR riêng) và `wrongCount`.
// - STARRED: khi chưa có `starred` -> filter rỗng -> startOrResume fallback về toàn bộ thẻ.
//   Sau khi NC12 thêm `starred` thì scope này mới lọc đúng.
// - HARD: chỉ dựa vào status === 'learning' (không có wrongCount trong model).
...
const selectNext = (session) => {
    ...
    // Chống lặp: bỏ thẻ vừa ra (trừ khi chỉ còn 1)
...
// ---- Chọn loại câu hỏi theo stage (fix B2) ----
const questionTypeFor = (session, cs) => {
    ...
    // Chỉ master mới có bước phơi nhiễm flashcard; cram vào ladder ngay
...
// ---- Sinh câu hỏi (ẩn correctAnswer khi trả về) ----
const generateQuestion = (session, cs, cardsById, allCards) => {
    ...
    // q.type có thể khác type ban đầu (buildMultipleChoice fallback buildWritten)
    return {
        flashcardId: cs.flashcardId.toString(), // lưu string để so sánh an toàn (fix B1)
        type: q.type,
        questionText: q.questionText,
        options: q.options || null,
        statement: q.statement || null,
        correctAnswer: q.correctAnswer ?? null,
        back: q.type === QUESTION_TYPE.FLASHCARD ? q.back : undefined, // chỉ flashcard mới kèm back (fix B1)
        answerSide: side
    }
}
...
// ---- Áp dụng kết quả (fix B2: tách flash outcome) ----
const applyOutcome = (session, cs, outcome) => {
    ...
    if (outcome === LEARN_OUTCOME.FLASH_KNOWN) {
        ...
        if (ladder.length === 0) {
            ...
            // trung tính: vào ladder, không đổi stage
    ...
    if (outcome === LEARN_OUTCOME.CORRECT) {
        ...
        // dontKnow: không phạt bậc, chỉ requeue gần
        cs.wrongTotal++
        session.stats.totalWrong++
        cs.due = session.step + LEARN_SESSION_LIMITS.GAP_DONTKNOW
    }
...
const grade = (session, payload) => {
    ...
    if (cur.type === QUESTION_TYPE.TRUE_FALSE) {
        ...
        return { isCorrect: ok, outcome: ok ? LEARN_OUTCOME.CORRECT : LEARN_OUTCOME.WRONG, fuzzy: false, correctAnswer: cur.correctAnswer }
    }
    ...
    return { isCorrect: ok, outcome: ok ? LEARN_OUTCOME.CORRECT : LEARN_OUTCOME.WRONG, fuzzy, correctAnswer: cur.correctAnswer }
}

// ---- Tạo summary kết thúc (NC10) ----
...
// issueNext đồng bộ (fix B3: bỏ qua thẻ đã bị xoá)
const issueNext = (session, cmap) => {
    while (true) {
        ...
        if (!card) {
            // thẻ đã bị xoá -> coi như xong, giải phóng slot
            cs.mastered = true
            cs.due = null
            continue
        }
...
const startOrResume = async (deckId, userId, { mode = LEARN_SESSION_MODE.MASTER, config } = {}) => {
    ...
    const needsMcOrTf =
        cfg.types.includes(QUESTION_TYPE.MULTIPLE_CHOICE) || cfg.types.includes(QUESTION_TYPE.TRUE_FALSE)
    ...
    // fix P1: tái dùng cards đã load, không query thêm
    const cmap = { all: cards, byId: new Map(cards.map((c) => [c._id.toString(), c])) }

    // fix B4: bọc create trong try/catch E11000
    let session
    try {
...
const answer = async (deckId, userId, payload) => {
    ...
    // Đồng bộ cardProgress tại mốc
    if (cs.mastered) await syncProgress(userId, cs.flashcardId, LEARNING_STATUS.REMEMBERED)
    else if (result.outcome !== LEARN_OUTCOME.CORRECT) await syncProgress(userId, cs.flashcardId, LEARNING_STATUS.LEARNING)

    const cmap = await cardsMap(deckId)

    // Kích hoạt thêm thẻ và kiểm tra hoàn thành
    activateCards(session)
